                    ...
                    <select class="form-select" name="plan_name" id="plan_name">
                        ...
                        @foreach ($plans as $plan)
                            <option value="{{ $plan->name }}"
                                {{ request('plan_name') == $plan->name ? 'selected' : '' }}>
                                {{ $plan->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="dropdown" style="margin-left: 1rem">
                ...
                <div class="input-group">
                    <select class="form-select" name="status" id="status">
                        ...
                        <option value="expiring" {{ request('status') == 'expiring' ? 'selected' : '' }}>Expiring
                        </option>
                    </select>
                </div>
            </div>
        ...
        <tbody>
            @php $counter = 1; @endphp @foreach ($agents as $agent)
                <tr class="text-center">
                    ...
                    <td>
                        ...
                        @php
                            ...
                            if ($expirationDate <= $today) {
                                ...
                                $status =
                                    '<span class="text-danger"><strong>(Expiring in ' .
                                    $daysLeft .
                                    ' days)</strong></span>';
                            } else {
                                // Plan is active
                    ...
                    <td><a href="{{ route('agent.show', ['id' => $agent->id, 'category' => 'today']) }}"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <strong>Balance Amount (&#8377;):</strong> {{ $agent->balance }}<br />
                    ...
                    <!-- recharge modal -->
                    <div class="modal fade" id="agentRechargeModal{{ $agent->id }}" tabindex="-1"
                        role="dialog" aria-labelledby="agentModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="agentModalLabel">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('agent.recharge', ['id' => $agent->id]) }}"
                                        method="POST">
                                        @csrf
                                        <div class="form-group">
                                            <label for="recharge_amount">Recharge Amount:</label>
                                            <input type="number" class="form-control" id="recharge_amount"
                                                name="recharge_amount" required>
                                        </div>
                    ...
                    <!-- plan update modal -->
                    <div class="modal fade" id="agentPlanUpdateModal{{ $agent->id }}" tabindex="-1"
                        role="dialog" aria-labelledby="agentModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="agentModalLabel">
                                        {{ $agent->full_name }}'s Current Plan - {{ $agent->plan_name }} , expiration:
                                        {{ $agent->expiration_date }}
                                    </h5>
                                </div>
                                <div class="modal-body">
                                    <form action="{{ route('agent.update-plan', ['id' => $agent->id]) }}"
                                        method="POST">
                                        @csrf
                                            ...
                                            <select class="form-control" id="plan_id" name="plan_id" required>
                                                @foreach ($plans as $plan)
                                                    <option value="{{ $plan->id }}"
                                                        {{ $plan->id == $agent->plan_id ? 'selected' : '' }}>
                                                        {{ $plan->name }} - Rs. {{ $plan->price }} for
                                                        {{ $plan->duration }} days
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <button type="submit" class="btn btn-primary m-2">Udpate</button>
                                    </form>
                                </div>
